...
export function buildCategories(items: CategorizableItem[]): CategoryGroup[] {
...
function ItemSlide({ item, basePath }: { item: CategorizableItem; basePath: string }) {
  return (
    <Link
    ...
    <div className="flex flex-col overflow-hidden rounded-xl border border-edge bg-surface">
      ...
      >
        ...
        <div className="absolute right-3 bottom-3">
          <span className="rounded-full border border-white/25 bg-black/30 px-2 py-0.5 font-mono text-[9px] text-white/90 backdrop-blur-sm">
            {category.items.length} {category.items.length === 1 ? itemLabel : itemLabelPlural}
          </span>
        </div>
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <CategoryTotalViews slugs={category.items.map((item) => item.slug)} />
        </div>
        ...
        <div className="group/carousel relative mt-4">
          ...
          >
            {category.items.map((item) => (
              <ItemSlide key={item.slug} item={item} basePath={basePath} />
            ))}
          </div>
